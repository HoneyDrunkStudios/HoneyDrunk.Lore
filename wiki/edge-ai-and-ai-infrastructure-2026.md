...
## 2026-08-26 compile additions: anonymous coding models and fine-tuning decision boundaries

### Source-backed claims
- RuntimeWire reports Ox Alpha reached major coding-agent usage through OpenCode and OpenRouter while the provider, model architecture, knowledge cutoff, and output-limit metadata remained unidentified; the clearest result is demand generated by distribution and price rather than proven model quality. Source: `raw/2026-08-26-rss-tldr-ai-anonymous-ox-alpha-processes-26t-tokens-on-opencode-breaks-ope.md`; page: [[ai-agent-harnesses]]. confidence: 1 secondary/source-backed report, last-confirmed 2026-08-26.
- OpenCode's route reportedly offered zero-day retention and no training use, while OpenRouter's listing reportedly retained prompts and completions but did not use them for training, making endpoint selection a data-governance decision even for the same named model. Source: `raw/2026-08-26-rss-tldr-ai-anonymous-ox-alpha-processes-26t-tokens-on-opencode-breaks-ope.md`; page: [[ai-coding-agent-security]]. confidence: 1 source, last-confirmed 2026-08-26.
- System Design Newsletter's fine-tuning guide frames fine-tuning as behavior specialization rather than reliable factual knowledge injection; prompting, RAG, agents, continued pretraining, and fine-tuning address different failure modes. Source: `raw/2026-08-26-rss-system-design-newsletter-fine-tuning-a-deep-dive.md`; page: [[agent-evaluation-and-benchmarks]]. confidence: 1 newsletter/practitioner source, last-confirmed 2026-08-26.
- The fine-tuning source says useful green flags include recurring structured-output failures, specific voice/persona needs, high steady inference volume, narrow tasks, and data-residency constraints, while red flags include frequently changing facts, insufficient quality data, shifting requirements, prompt sufficiency, and need for instant rollback. Source: `raw/2026-08-26-rss-system-design-newsletter-fine-tuning-a-deep-dive.md`. confidence: 1 source, last-confirmed 2026-08-26.
- The same source warns that training can introduce production failure modes such as overfitting, catastrophic forgetting, and safety degradation, and that chat templates must match the model family because wrong templates can silently degrade responses. Source: `raw/2026-08-26-rss-system-design-newsletter-fine-tuning-a-deep-dive.md`. confidence: 1 source, last-confirmed 2026-08-26.

### Typed entities
- model/provider route: Ox Alpha
- platform: OpenCode
- platform: OpenRouter
- concept: endpoint retention terms
- method: fine-tuning
- method: continued pretraining / CPT
...
### Explicit relationships
- Model-routing decisions depend-on endpoint terms, provider identity, tool-call reliability, cost, context length, and local task quality.
- Fine-tuning complements prompting and RAG when the problem is stable behavior, not current knowledge.
- RAG supersedes fine-tuning for frequently changing facts because retrieval can update without retraining weights.
- Training safety depends-on evaluation before and after tuning because specialization can degrade prior alignment or broader capability.
- Chat-model fine-tuning depends-on the correct chat template; template mismatch can break instruction-role structure without obvious errors.

### HoneyDrunk implications
- Do not treat a free OpenAI-compatible endpoint as safe for proprietary code until provider identity, data handling, retention, training use, and tool-call behavior are confirmed.
- For HoneyDrunk fine-tuning decisions, require a written failure-mode classification: behavior, knowledge, tool access, workflow, or cost. Choose prompt/RAG/agent/fine-tune only after that classification.
- Any fine-tuned model evaluation should include safety regression, format adherence, task accuracy, chat-template validation, and rollback plan.
