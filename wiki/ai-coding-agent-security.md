...
## 2026-07-04 compile additions: phantom domains, cyber safeguards, uv malware checks, and practical threat modeling

### Source-backed claims
- Unit 42 describes "phantom squatting" as adversaries registering LLM-hallucinated web domains for legitimate brands, turning model-generated URLs into a zero-reputation supply-chain and phishing vector. Source: `raw/2026-07-04-web-phantom-squatting-ai-hallucinated-domains-as-a-software-supply-chain-vector.md`. confidence: 1 security-research source, last-confirmed 2026-07-04.
- Unit 42 reports 685,339 URL queries across 913 brands and two LLM families, yielding 2.1 million URLs, 13,229 malicious URLs, and roughly 250,000 unregistered hallucinated phantom domains. Source: `raw/2026-07-04-web-phantom-squatting-ai-hallucinated-domains-as-a-software-supply-chain-vector.md`. confidence: 1 vendor security-research source, last-confirmed 2026-07-04.
- Unit 42's observed cases show proactive lead time between hallucination detection and adversary registration, including 23-day and 51-day adversarial exploitation windows in postal-themed campaigns. Source: `raw/2026-07-04-web-phantom-squatting-ai-hallucinated-domains-as-a-software-supply-chain-vector.md`. confidence: 1 source, last-confirmed 2026-07-04.
- Anthropic's Fable 5 safeguards source categorizes cyber classifier behavior into prohibited use, high-risk dual use, low-risk dual use, and benign use, with high-risk dual use blocked until stronger controls can limit access to known good actors. Source: `raw/2026-07-04-web-more-details-on-fable-5-s-cyber-safeguards-and-our-jailbreak-framework.md`; page: [[claude-platform-2026]]. confidence: 1 Anthropic source, last-confirmed 2026-07-04.
- Anthropic proposes a Cyber Jailbreak Severity scale using capability gain, breadth, ease of weaponization, and discoverability, with final severity able to be raised above the calculated floor when real-world risk is underestimated. Source: `raw/2026-07-04-web-more-details-on-fable-5-s-cyber-safeguards-and-our-jailbreak-framework.md`. confidence: 1 source, last-confirmed 2026-07-04.
- Astral's `uv audit` is a preview uv-native vulnerability/adverse-status scanner, while opt-in `UV_MALWARE_CHECK=1` performs an OSV-based malware advisory lookup during sync and terminates before known malicious locked packages install. Source: `raw/2026-07-04-web-vulnerability-and-malware-checks-in-uv.md`. confidence: 1 project/vendor source, last-confirmed 2026-07-04.
- The uv source highlights a lockfile-specific malware risk: quarantine can remove a malicious package from the package index while direct object-storage references in lockfiles may still be installable unless the installer checks malware advisories at sync time. Source: `raw/2026-07-04-web-vulnerability-and-malware-checks-in-uv.md`. confidence: 1 source, last-confirmed 2026-07-04.
- Soatok's threat-model guide reinforces that a useful security review should state assets, adversaries/failures, attack paths, mitigations, assumptions, asset relationships, and out-of-scope threats. Source: `raw/2026-07-04-web-soatok-s-informal-guide-to-threat-models-dhole-moments.md`. confidence: 1 practitioner source, last-confirmed 2026-07-04.

### Typed entities
- attack class: phantom squatting
- concept: LLM-hallucinated domain
- concept: zero-reputation bypass
- concept: adversarial exploitation window / AEW
- product: Claude Fable 5
- framework: Cyber Jailbreak Severity / CJS
- package manager: uv
- command: `uv audit`
- environment variable: `UV_MALWARE_CHECK`
- database: OSV
- advisory type: MAL advisory
- concept: lockfile direct object-storage reference
- concept: threat model

### Explicit relationships
- Phantom squatting extends slopsquatting from package names to web domains and API endpoints generated by LLMs.
- Agentic web fetching depends-on URL verification because an autonomous agent may retrieve, process, or trust hallucinated infrastructure without a human pause.
- Fable 5 cyber safeguards depend-on classifier policy, access controls, model training, monitoring, and jailbreak severity triage; classifiers alone do not supersede runtime controls.
- `uv audit` complements CI dependency scanning by using uv's lock/context, while malware sync checks complement audit because malicious packages require pre-install blocking and incident response.
- Threat modeling complements scanner output by stating which assets and assumptions the controls actually protect.

### HoneyDrunk implications
- Treat LLM-suggested domains, docs links, webhook endpoints, and package/service URLs as untrusted until resolved against primary documentation or approved registries.
- Add hallucinated-domain risk to agent browser/research threat models, especially when agents can fetch schemas, scripts, docs, or deployment URLs.
- For Python tooling, evaluate `uv audit` and `UV_MALWARE_CHECK=1` on representative repos, including lockfile behavior, false positives, offline/cache impact, and CI ergonomics.
- Classify security-agent findings with uplift, breadth, weaponization effort, and discoverability; this gives HoneyDrunk a clearer risk vocabulary than "jailbreak found".
