...
title: "Don't let the LLM speak, just probe it. - by James Padolsey"
author: "j11y.io"
...
The knobs to play with are the seed token itself (e.g. `"Assessment: ___"`

or `"Criteria met? ___"`

), its exact position relative to the criterion and content, and the precise layer(s) of extraction. Often it is not the final layer that holds most value, but somewhere after the middle. This needs lots of experimentation to figure out, and depends on the underlying model and problem domain.
