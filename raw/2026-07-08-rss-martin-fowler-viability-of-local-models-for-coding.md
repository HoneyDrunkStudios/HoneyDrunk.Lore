...
Goals Model runnability Response speed Tool calling success Quality of outcomes Goals Model LLM runtime Quantization Architecture Format Reasoning Model Hardware RAM Processing power Memory bandwidth Hardware Harness Size and  complexity of task Context window size prefills
(system prompt, tools, ...) varying tool schemas influence... impacts 
need for provides templates to improve can reduce improves impacts
need for better hardware fit leads to better... increases
KV cache in... reduces
need for fills up
faster differ in optimisation for... 
...
windows of maximum 64K, harnesses OpenCode and Pi with zero Skills and MCP servers active) 
...
here ) 
Claude Code: old_string and new_string (at least that's what
